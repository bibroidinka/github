...
scrollingFrame.Visible = false  -- Скрыт изначально
...
function CreateLabel(text, pos)
	...
	label.Position = pos
	label.Size = UDim2.new(1, -10, 0, 0)  -- Ширина от прокрутки, высоту можно изменить ниже
	label.Parent = scrollingFrame

	-- Ждем один кадр для вычисления TextBounds
	task.wait()
	local neededHeight = label.TextBounds.Y
	label.Size = UDim2.new(1, -10, 0, neededHeight + 10)
	scrollingFrame.CanvasSize = UDim2.new(0, 0, 0, neededHeight + 20)  -- Обновляем CanvasSize
